...
local keyboardIdentifier = '1B97874A'
...
sendToAHK = function (key)
   if key == 'v' then
      ...
      local file = io.open("E:\\dump\\keypressed.txt", "w") -- writing this string to a text file on disk is probably NOT the best method. Feel free to program something better!
...
lmc_set_handler('MACROS', function(button, direction)
   ...
   buttonChar = config[button]
	if type(buttonChar) == "string" then
            if buttonChar == "p" then
                lmc_send_keys('{F23}')
            end
--                print('Your key ID number is:   ' .. button)
				print('It was assigned string:    ' .. buttonChar)
            if lasttwo == "bb" then
				   sendToAHK("bb")
               lasttwo = ''
            elseif lasttwo == "rr" then
				   sendToAHK("rr")
               lasttwo = ''
            elseif buttonChar == "space" then
               lasttwo = ''
            else
               lasttwo = lasttwo .. config[button]
               print(lasttwo)
            end
