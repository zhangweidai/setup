...
class App(object):
    ...
    def __init__(self):
        ...
        self.player2 = drag_text.Character(self.screen_rect.center)
    ...
    def event_loop(self):
        ...
        for event in pg.event.get():
            if event.type == pg.QUIT or self.keys[pg.K_ESCAPE]:
                ...
                self.player2.check_click(event.pos)

            elif event.type == pg.MOUSEBUTTONUP and event.button == 1:
                self.player2.click = False
...
def main():
    ...
    bar = pg.display.set_mode(SCREEN_SIZE)
    App().main_loop()
    pg.quit()
    sys.exit()
